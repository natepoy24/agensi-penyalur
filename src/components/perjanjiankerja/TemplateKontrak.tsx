...
export default function TemplateKontrak({ formData, pasalList, paperSize = "f4" }: TemplateKontrakProps) {
    ...
    const bodyTextSize = isA4 ? "text-[11px]" : "text-[12px]";
    const subPointTextSize = isA4 ? "text-[10.5px]" : "text-[11.5px]";
    const pasalSpacing = isA4 ? "space-y-3" : "space-y-3.5";
        ...
        <div className={`w-[210mm] bg-white relative px-10 ${isA4 ? "py-6" : "py-8"} print:w-full print:shadow-none print:px-8 print:py-6 z-10 text-black box-border`}>
            ...
            <div className="relative z-10">
                ...
                <div className="text-center mb-3 mt-1">
                    <h2 className="text-base font-bold uppercase underline underline-offset-4 tracking-wide text-slate-900 leading-tight">
                        Surat Perjanjian Kerja
                    </h2>
                    <p className="text-[11px] font-semibold text-slate-600 font-mono mt-0.5">
                ...
                <div className={`${bodyTextSize} text-slate-800 mb-3 leading-snug`}>
                    ...
                    <p className="italic text-[10px] text-slate-600 mb-2">Selanjutnya disebut sebagai <strong>PIHAK KESATU (Pengguna Jasa)</strong>.</p>
                    ...
                    <p className="italic text-[10px] text-slate-600 mb-3">Selanjutnya disebut sebagai <strong>PIHAK KEDUA (Perusahaan Penyalur)</strong>.</p>

                    {/* KETERANGAN PEKERJA BOX */}
                    <div className="bg-slate-50 border border-slate-300 p-2.5 rounded-lg mb-3">
                        <p className="font-bold underline text-[11px] mb-1 text-slate-900">Keterangan Pekerja yang Ditempatkan:</p>
                    ...
                    {/* SELURUH PASAL 1 S/D PASAL 6 DENGAN SLICING SEBELUM PASAL 4 */}
                    <div className={`${pasalSpacing} my-3`}>
                        {pasalList.map((pasal, pIdx) => {
                            const isPasal4 = pIdx === 3;
                            return (
                                <div
                                    key={pasal.id || pIdx}
                                    className={`space-y-1 ${isPasal4 ? "html2pdf-break-before pt-6 print:pt-8" : ""}`}
                                    style={isPasal4 ? { pageBreakBefore: "always", breakBefore: "page" } : undefined}
                                >
                                    ...
                                    <div className="pl-6 space-y-0.5">
                                        {pasal.poin.map((teksPoin: string, idx: number) => (
                                            <div key={idx} className={`flex items-start gap-2 ${subPointTextSize}`}>
                ...
                <div className="break-inside-avoid mt-4" style={{ breakInside: "avoid", pageBreakInside: "avoid" }}>
                    ...
                    <div className="flex justify-between px-10 mb-2">
                        <div className="text-center w-56">
                            <p className={`${bodyTextSize} mb-16 font-semibold text-slate-800`}>CV Jasa Mandiri</p>
                            <p className={`${bodyTextSize} font-bold underline uppercase text-slate-900`}>( Atep Jaenudin )</p>
                        </div>
                        <div className="text-center w-56">
                            <p className={`${bodyTextSize} mb-16 font-semibold text-slate-800`}>Pengguna Jasa</p>
                            <p className={`${bodyTextSize} font-bold underline uppercase text-slate-900`}>
                                ( {formData.namaMajikan || "_________________________"} )
                            </p>
                            {formData.tipeMajikan === "perusahaan" && formData.namaInstansi && (
                                <p className="text-[11px] font-bold text-emerald-800 uppercase mt-0.5 leading-tight">
